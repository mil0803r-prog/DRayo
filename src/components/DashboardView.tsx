...
import { Sale, MetaAdExpense, Product, TabType, DailySaleRecord } from '../types';
import {
  DollarSign,
  TrendingUp,
  Megaphone,
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // 5. Inventory Overview Stats & Total Product Valuation
  ...
  // 6. DYNAMIC MONTH DETECTION (100% Real-time and connected to all dates)
  ...
  const monthlyData = months.map((m) => {
    ...
    // Break-even revenue needed = Ad Spend + COGS
    const breakEvenRevenue = monthAdSpend + monthCogs;
  ...
  // 7. BREAK-EVEN (PUNTO DE EQUILIBRIO) OVERALL CALCULATIONS
  const totalBreakEvenRevenue = totalAdSpend + totalCOGS;
  ...
  const targetBreakEvenUnits =
    avgContributionMargin > 0 && totalAdSpend > 0
      ? Math.ceil(totalAdSpend / avgContributionMargin)
      : 0;
      ...
      <div className="bg-slate-900 text-white p-4.5 rounded-2xl shadow-md flex flex-wrap items-center justify-between gap-3 border border-slate-800">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          >
            ...
            <span>Ventas WhatsApp ({dailyRecords.length})</span>
          </button>
          <button
